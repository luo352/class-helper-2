...
function bytesToHex(bytes: number[]): string {
  ...
  for (let i = 0; i < bytes.length; i++) {
    result += HEX_CHARS[(bytes[i] >> 4) & 0x0f] + HEX_CHARS[bytes[i] & 0x0f]
  }
  return result
}
...
function sha256(message: string): string {
  ...
  const originalMessageLength = bytes.length * 8
  ...
  for (let i = 7; i >= 0; i--) {
    bytes.push((originalMessageLength >> (8 * i)) & 0xff)
  }
  ...
  for (const h of [h0, h1, h2, h3, h4, h5, h6, h7]) {
    hashBytes.push((h >> 24) & 0xff, (h >> 16) & 0xff, (h >> 8) & 0xff, h & 0xff)
  }
